...
const Husky = ({tokenidprop}) => {
    ...
    const imageSrc = `https://huskies.s3.eu-west-2.amazonaws.com/images/${tokenId}.png`
    ...
    let responseString
        ...
        try{
            ...
            if(osResponse.detail){
                setHuskyProv(<ListGroup.Item>{`${osResponse.detail}.. by those bastards at OS. Wait a second then try again.`}</ListGroup.Item> )
                setloadedFlag(true)
            }
